...
async function rodarVerificacaoAgora() {
    ...
    try {
        ...
        if (dados.sucesso) {
            ...
            mensagem.textContent = `✅ Concluído: ${r.totalVerificados} verificado(s), ${r.alteracoesEncontradas} alteração(ões), ${r.imagensCapturadas} imagem(ns) capturada(s), ${r.naoConseguiuAcessar} não confirmado(s) (link bloqueado/inacessível, nada foi alterado), ${r.semLinkOriginal} sem link original cadastrado (pulado), ${r.falhas} falha(s).`;
            carregarAlteracoes();
